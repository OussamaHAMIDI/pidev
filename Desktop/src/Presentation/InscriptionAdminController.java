...
public class InscriptionAdminController implements Initializable {
    ...
    private String absolutePathphotoProfil = null;
    private String absolutePathPhotoPermis = null;
    ...
    private void onSetAction_importer(ActionEvent event) throws IOException {
        ...
        if (selected_photo != null) {
            if ((selected_photo.length() / 1024) / 1024 < 2.0) {
                absolutePathphotoProfil = selected_photo.getAbsolutePath();
    ...
    private void inscrireClicked(ActionEvent event) {
        ...
        if (verif()) {
            if (type.getValue().equals("Artisan") && absolutePathPhotoPermis == null) {
...
//              String mdp = Utils.hashPassword(password.getText(), salt);
                ...
                User u = new User(us.getNextId(), username.getText(), mdp, EtatUser.Active, TypeUser.valueOf(type.getValue()), nom.getText(), prenom.getText(),
                        ...
                        null, null, "role va etre ajouter dans la methode ajouterUser", code, absolutePathphotoProfil);

                u = us.ajouterUser(u);
                if (u != null) {

                    if (type.getValue().equals("Artisan") && absolutePathPhotoPermis != null) {
                        u = us.addPhotoArtisan(u, absolutePathPhotoPermis);
                    }
                    ...
                    if (u.getPhoto() != null) {
                        img = new Image(Utils.dir + u.getPhoto());
                    }
                    Utils.showTrayNotification(NotificationType.CUSTOM, "Succés d'ajout", null, "L'utilisateur a été ajouté avec succés", u.getPhoto(), 6000);
    ...
    private void onSetAction_importerPermis(ActionEvent event) throws IOException {
        ...
        if (selected_photo != null) {
            if ((selected_photo.length() / 1024) / 1024 < 2.0) {
                absolutePathPhotoPermis = selected_photo.getAbsolutePath();
                BufferedImage bufferedImage = ImageIO.read(selected_photo);
                WritableImage image = SwingFXUtils.toFXImage(bufferedImage, null);
